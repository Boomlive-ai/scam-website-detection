...
#     Returns False for now until MySQL/PHP API is implemented
#     """
#     print("[INFO] url_in_reporting_database() is disabled - Firebase not configured")
#     return False
...
"""
Utils.py - URL security checking utilities with optimized functions
"""
...
try:
    ...
    TEMPORARY_DOMAIN_PLATFORMS = ['ngrok.io', 'herokuapp.com', 'netlify.app']
...
def is_ip_address(url):
    """Check if URL uses IP address instead of domain - returns 1 or 0"""
...
def has_malicious_extension(url):
    """Check for malicious file extensions - returns 1 or 0"""
    _, ext = os.path.splitext(url)
    malicious_extensions = [
        '.exe', '.dll', '.bat', '.cmd', '.scr', '.js', 
        '.vbs', '.hta', '.ps1', '.jar', '.sh', '.bin', '.msi'
    ]
    return 1 if ext.lower() in malicious_extensions else 0

def is_temporary_domain(url):
    """Check if domain is from temporary hosting platform"""
    for temp_domain in TEMPORARY_DOMAIN_PLATFORMS:
        if temp_domain in url.lower():
            return True
    return False

def is_valid_ip(text):
    """Check if text is valid IPv4 or IPv6 address"""
    try:
        ipaddress.ip_address(text)
        return True
    except ValueError:
...
# ==================== TOP 1 MILLION DATABASE ====================

@lru_cache(maxsize=1)
def load_top1million_domains():
    """Load Top 1M domains into memory once (cached)"""
    domains = set()
    try:
        with open('top-1million-sites.csv', 'r') as f:
            reader = csv.reader(f)
            for row in reader:
                if len(row) > 1:
                    domains.add(row[1].lower())
                    domains.add(f"www.{row[1].lower()}")
        print(f"✓ Loaded {len(domains)} domains into memory")
    except FileNotFoundError:
        print("⚠ top-1million-sites.csv not found")
    except Exception as e:
        print(f"Error loading top 1M database: {e}")
    return domains

def check_top1million_database(url):
    """Check if URL is in top 1M sites (slow method)"""
    try:
        with open('top-1million-sites.csv', 'r') as f:
            reader = csv.reader(f)
            for row in reader:
                if len(row) > 1:
                    if url in row[1] or url in f"<https://www.{row>[1]}":
                        return True
    except:
        pass
    return False
...
def check_top1million_database_2(url):
    """Check if domain is in top 1M sites (extracts domain first)"""
    try:
        domain = urlparse(url).netloc
        if not domain:
            domain = url.split('/')[0]
        ...
        with open('top-1million-sites.csv', 'r') as f:
            ...
            for row in reader:
                if len(row) > 1:
                    if domain == row[1] or domain == f"www.{row[1]}":
                        return True
    except:
        pass
    return False
...
def get_days_since_creation(domain, months):
    """Check if domain is older than X months"""
    if whois is None:
        return None
...
def checkLocalBlacklist(url):
    """Check if URL is in local blacklist file"""
    try:
        with open('blacklisted_sites.txt', 'r') as file:
            for line in file:
                website = line.strip()
                if url == website:
                    return True
        ...
        print(f"Local blacklist error: {e}")
    return False

def checkURLVoid(url):
    """Check URLVoid for blacklist count"""
    if BeautifulSoup is None:
        return 0
    
    try:
        scan_url = f"https://www.urlvoid.com/scan/{url}"
        response = requests.get(scan_url, timeout=(5, 10))
        soup = BeautifulSoup(response.content, 'html.parser')
        span_tag = soup.find('span', class_="label label-danger")
...
def check_ip_in_ipsets(ip):
    """Check if IP is in local blocklist IPsets"""
    try:
        ip_obj = ipaddress.ip_address(ip)
        ipset_directory = "blocklist-ipsets/IpSets"
        
        if not os.path.exists(ipset_directory):
            return False
...
def query_params_count(url):
    ...
    parsed = urlparse(url)
    query_params = parsed.query.split('&')
    return 0 if query_params[0] == '' else len(query_params)

def path_tokens_count(url):
    """Count path tokens in URL"""
    parsed = urlparse(url)
...
def has_special_characters(url):
    """Check for special characters - returns 1 or 0"""
    special_chars = ['@', '!', '#', '$', '%', '^', '&', '*', '_', '+']
    return 1 if any(char in url for char in special_chars) else 0

def getInputArray(url):
    """Get feature array for ML model"""
    result = [
        get_domain_length(url),
        get_domain_entropy(url),
        is_ip_address(url),
        has_malicious_extension(url),
        query_params_count(url),
        path_tokens_count(url),
        hyphens_count(url),
        digits_count(url),
        has_special_characters(url)
    ]
    return result
...
def calculate_url_similarity(url1, url2):
    """Calculate similarity score between two URLs (0-10)"""
    ...
    try:
        ...
        if max_len == 0:
            return 0
        similarity_score = (1 - levenshtein_distance / max_len) * 10
        return similarity_score
    except:
        return 0

def find_target_urls(fake_url, similarity_score=7):
    """Find potential typosquatting targets"""
    if Levenshtein is None:
        return []
        ...
        with open('top-1million-sites.csv', 'r') as f:
            ...
            for row in reader:
                if len(row) > 1:
                    if calculate_url_similarity(domain, row[1]) > similarity_score:
                        similar_urls.append(row[1])
                        if len(similar_urls) >= 10:  # Limit results
                            break
    except Exception as e:
        print(f"Error finding target URLs: {e}")
